describe("snippet", function()
  ...
  describe("makeSnippet", function()
    it("should return a function with docstring", function()
      assert.equals(
        snippet.makeSnippet("hello_world", "hello world"),
  ...
  describe("getQuery", function()
    it("checking function name and docstring query", function()
      ...
      vim.api.nvim_buf_set_lines(
        ...
        -1,
        false,
        { "def testFunc():", '\t""" hello world"""', "\tpass" }
      )
      local functioNameQuery =
        helper.getQuery("(function_definition name: (identifier)@capture)", bufnr)[1]
      local docstringQuery = helper.getQuery(
        "(function_definition body: (block (expression_statement (string)@capture)))",
        bufnr
      )[1]
      assert.equals(
        vim.treesitter.get_node_text(functioNameQuery, bufnr),
        "testFunc"
      )
      assert.equals(
        vim.treesitter.get_node_text(docstringQuery, bufnr),
        '""" hello world"""'
      )
      vim.cmd("bd!")
    end)
    ...
    it("should get all function and docstring in file", function()
      ...
      for i = 1, #functionName do
        vim.api.nvim_buf_set_lines(bufnr, -1, -1, false, {
          string.format("def %s():", functionName[i]),
          "\t" .. docstring[i],
          "\tpass",
        })
      end
      ...
      for i = 1, #functioNameQuery do
        assert.equals(
          vim.treesitter.get_node_text(functioNameQuery[i], bufnr),
          functionName[i]
        )
        assert.equals(
          vim.treesitter.get_node_text(docstringQuery[i], bufnr),
          docstring[i]
        )
      end
      vim.cmd("bd!")
