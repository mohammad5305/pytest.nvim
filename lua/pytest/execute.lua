...
function execute.setSignCol(id, state, bufnr, line)
  if vim.trim(state) == "PASSED" then
    vim.cmd(
      string.format("sign place %s group=pytest name=PytestSucces line=%s", id, line)
    )
  else
    vim.cmd(
      string.format("sign place %s group=pytest name=PytestFailed line=%s", id, line)
    )
  end
end
...
function execute.processReport(report)
  ...
  for _, line in ipairs(report) do
    if string.find(line, ":") then
      ...
      local functionName, status =
        unpack(vim.split(reportResult[#reportResult], " "))
      table.insert(reportTbl, {
        ["functionName"] = string.match(functionName, "[A-Za-z_]+"),
        ["status"] = status,
      })
    end
  end
